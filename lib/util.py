...
def setJpName(objName, jpName):
  maya.cmds.addAttr(objName, dt="string", ln="jpName", h=False, k=False, s=True)
...
def setAtAttr(objName, attrName, attrType, message):
  maya.cmds.addAttr(objName, at=attrType, ln=attrName, h=False, k=False, s=True)
  maya.cmds.setAttr("%s.%s" % (objName, attrName), message)

def setDtAttr(objName, attrName, attrType, message):
  maya.cmds.addAttr(objName, dt=attrType, ln=attrName, h=False, k=False, s=True)
